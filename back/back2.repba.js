...
url.slidecount = url.searchParams.has("i") ? Number(url.searchParams.get("i")) : 80;
url.slidereduce = url.searchParams.has("s") ? Number(url.searchParams.get("s")) : 2;
...
url.timemain = url.searchParams.has("j") ? Number(url.searchParams.get("j")) : 4;
...
function resetcanvas()
{
    ...
    */
    ...
    var slicewidth = Math.lerp(1,2,context.zoomobj.berp());
...
var ContextObj = (function ()
{
    function init()
    {
        ...
        slices.data_.push({title:"Thumbnail", path: "THUMBNAIL", func: thumbnail})
            ...
            for (var n = 0; n < 1; n++)
            {
                ...
                if (m == sliceobj.length)
                {
                    ...
                    for (var m = m0; m < m1; ++m)
                    {
                        ...
                        let pinchwidth = Math.max(sliceobj[m+1]?sliceobj[m+1].xx-x:1,1); 
                ...
                if (!context.describe && !url.hide && !context.screenshot)
                    thumbobj.getcurrent().draw(context, rect, 0, 0);
                
                if (context.zoomobj.current() && !context.screenshot)
                {
                    context.shadowOffsetX = 0;
                    context.shadowOffsetY = 0;
                    var a = new Fill("black");
                    a.draw(context, new rectangle(rect.width-HNUB,YNUB,HNUB,rect.height-YNUB), 0, 0);
                    var a = new CurrentVPanel(new Fill("white"),ALIEXTENT*2);
                    a.draw(context, new rectangle(rect.width-HNUB,YNUB,HNUB,rect.height-YNUB), context.rowobj, 0);
                    var s = context.sliceobj.data_.length;
                    var e = context.slicefirst/s;
                    var k = context.count/s; 
                    var j = e*rect.width;
            ...
            [
                ...
                [
                    ...
                    [
                        ...
                        new Layer(
                        [
                            (_4cnvctx.newimage == -1) ? new Fill("black"):0,
