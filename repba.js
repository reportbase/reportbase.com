...
const MAXVIRTUALSIZE = SAFARI?7000000:12000000
...
function accutime(context)
{
    ...
    var init = () => 
    {
        setTimeout(function () 
        {
            ...
            {
                init();
                if (context.thumbselect.length == context.splitpanels) 
                    return;
            } 
        }, 10);
    }
...
},
{
    ...
    enabled : function() { return 1; },
	panmove: function (context, rect, x, y) { },
	panstart: function (context, rect, x, y) { context.speed = 0; },
	panend: function (context, rect, x, y) { }
},
{
    name: "BOSS",
...
function toggledebug()
{
    ...
    [
        ...
        {title: "Canvas Length", value: canvaslst.length.toFixed(0)},
...
},
{
	...
	tap: function (context, rect, x, y, shift, ctrl)
	{
        ...
        context.isthumbrect = context.thumbrect && context.thumbrect.hitest(x,y);
        url.footindex = context.isthumbrect ? 2 : 1;
...
[
{
    ...
    draw: function (context, rect, user, time)
    {
        ...
        var whiterect = new StrokeRect("white");
...
function resetcanvas()
{
    ...
    for (var n = 0; n < canvaslen; ++n)
	{
        var fullcaa = canvaslst[n];
        var cxx = fullcaa.getContext('2d');
		fullcaa.height = context.canvas.height;
		fullcaa.width = context.bwidth;
...
window.addEventListener('focus', (event) => { resize(); });
window.addEventListener('blur', (event) => { });
window.addEventListener('screenorientation', function () { resize(); });
...
var YollPanel = function ()
{
    this.draw = function (context, rc, user, unused)
    {
        ...
        yoll();
        clearInterval(globalobj.timeyoll);
        globalobj.timeyoll = setInterval(function () { yoll(); }, TIMEMAIN);
    };
...
[
	...
	new function ()
	{
        ...
        this.swipeleftright = function (context, rect, x, y, type)
        {
            ...
            context.timeswipe = setTimeout(function()
            {
                _4cnvctx.timeback = (type == "swipeleft") ? 0 : 1;
                accutime(_4cnvctx); 
            }, JULIETIME);
        };
		...
		this.draw = function (context, rect, user, time)
		{
            ...
            var project = k[0];
            var projects = k[1];
            var e = url.cols == 1 ? 50 : 0;
            var a = new Layer(
            [
                new Fill(HEADCOLOR),
                new ColA([0,ALIEXTENT,ALIEXTENT,24*url.cols+e,ALIEXTENT,ALIEXTENT,0],
                [
                    0,
                    new Layer(
                    [
           ...
           a.draw(context, rect, [0,project,0,str,0,projects,0], time);
		};
	},
];
